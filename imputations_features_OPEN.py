...
filepath='/data/scratch/bt23801/rrbs/methylkit_tryout_blood/open_genes_model/cpgs_open_genes.csv' #filepath to the cpgs related to ageing genes 
...
for percentage in percentages: 
    ...
    data_merged=merging_df(data_imputed,filepath2,percentage,'open')
    print('done')
    
    #looping through each fold
    for fold in folds: 
        #filepath for the merged with age imputed file 
        filepath_merged=f'merged_{percentage}_open_rrbs_only.csv'
        #running the feature selection function from publication
        feature_selection(filepath_merged,percentage,fold,'open')
        ...
        results=pd.read_csv(f'~/research_project/src/imputation_tests_open_rrbs/feature_selection/results_all_feature_selections_{percentage}__{fold}CV.csv')
        #bootstrapping 100 times each feature selection option 16 of them as a testing method
        bootstrap_elastic(16,100,data,results,percentage,fold,'open')
