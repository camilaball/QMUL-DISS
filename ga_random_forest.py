...
def training_creatures(X_train, y_train, X_test, y_test, initial_population):
    for i, r in initial_population.iterrows():
        if initial_population.at[i, 'Score'] > 0 or math.isnan(initial_population.at[i, 'Score']):
            ...
            cur_feat = r[1]
            model = RandomForestRegressor().fit(X_train[cur_feat], y_train)
...
def ga(X_train, y_train, X_test, y_test):
    ...
    for i in range(10):
        ...
        grid = GridSearchCV(estimator=rf, param_grid=param_grid, scoring='r2', cv=10, n_jobs=-1)
        grid.fit(X_train[cur_creatures['Feat'][i]].to_numpy(), y_train)
        best_parameters = grid.best_params_

        final_clock_model = RandomForestRegressor(
...
#         counter += 1
    ...
    final_cpgs_df.to_csv('ga_models/rf_rf/results_20_cv2_open_rf_rf_rrbs.csv', index=False)